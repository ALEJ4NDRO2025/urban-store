...
export default function Navbar() {
  ...
  const [loggedIn, setLoggedIn] = useState(false)  // ← solo booleano, no depende de 'user'
  const [email, setEmail]       = useState('')
  const [hovering, setHovering] = useState(null)
  const itemCount = useCartStore((state) => state.itemCount)

  useEffect(() => {
    // ✅ Solo necesita 'access' para saber si hay sesión
    const token = localStorage.getItem('access')
    if (token) {
      setLoggedIn(true)
      // Intenta leer email del objeto user si existe
