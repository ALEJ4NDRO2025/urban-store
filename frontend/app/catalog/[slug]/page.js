...
import { useCartStore } from '../../lib/cartStore'  // ← conecta al store real
  ...
  return (
    <div style={{ ...styles.pageSection, maxWidth: '1100px', margin: '0 auto', padding: '40px 24px' }}>
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '48px' }}>
        ...
        {/* ── GALERÍA ── */}
        <div>
          <div style={{ width: '100%', aspectRatio: '1/1', backgroundColor: c.card, borderRadius: '8px', border: `1.5px solid ${c.border}`, overflow: 'hidden', marginBottom: '16px' }}>
            ...
            <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
              {product.images.map((img, index) => (
                <div key={index} onClick={() => setSelectedImage(index)}
                  style={{ width: '72px', height: '72px', borderRadius: '6px', overflow: 'hidden', cursor: 'pointer', border: `2px solid ${selectedImage === index ? c.primary : c.border}`, opacity: selectedImage === index ? 1 : 0.6 }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
          ...
          {addingError && (
            <p style={{ color: c.error, fontSize: '13px', margin: 0 }}>{addingError}</p>
          )}
          ...
          {/* Ir al carrito si ya agregó */}
          {added && (
            <button
              onClick={() => router.push('/carrito')}
              style={{ padding: '12px', backgroundColor: 'transparent', color: c.primary, border: `1px solid ${c.primary}`, borderRadius: '4px', cursor: 'pointer', fontWeight: 'bold', fontSize: '14px' }}
            >
              Ver carrito →
            </button>
          )}
